...
class Config {
	...
	*/
	static public function getConfigLineByName($name) {
		try {
			$stmt = DB::getInstance()->prepare("SELECT name,value,last_modified FROM Configuration WHERE name=?");
	...
	*/
	static public function getConfigValueByName($name) {
		try {
			$stmt = DB::getInstance()->prepare("SELECT value FROM Configuration WHERE name=?");
			$stmt->execute(array($name));
			$rows = $stmt->fetch(PDO::FETCH_ASSOC);
		} catch(PDOException $e) {
			echo $e->getMessage();
	...
	static public function writeConfigLine($name, $value) {
		...
		try {
			$stmt = DB::getInstance()->prepare("	INSERT INTO 
									Configuration 	(name, value, last_modified) 
								VALUES 
										(:name, :value, NOW()) 
								ON DUPLICATE KEY 
									UPDATE 
